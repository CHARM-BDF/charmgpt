...
// PUT /api/graph/:conversationId/edges/bulk-update - Bulk update edges
router.put('/:conversationId/edges/bulk-update', async (req: Request, res: Response) => {
  try {
    const { conversationId } = req.params;
    const { edges } = req.body; // Array of edge objects with composite IDs
    const loggingService = req.app.locals.loggingService as LoggingService;
    
    loggingService.log('info', `Bulk updating ${edges.length} edges in conversation: ${conversationId}`);
    ...
    // Bulk update edges
    const result = await db.bulkUpdateEdges(edgesWithGraphId);
    
    // Save state for undo/redo
    await db.saveState(
      graphProject.id,
      `Bulk updated ${result.updated} edges`,
      await db.getCurrentGraphState(conversationId)
    );
    ...
    console.error('Error bulk updating edges:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    });
