...
async function runTests() {
  ...
  // Load providers dynamically to avoid TypeScript import issues
  const { AnthropicProvider } = require('./dist/server/services/llm/providers/anthropic');
  const { OpenAIProvider } = require('./dist/server/services/llm/providers/openai');
  const { GeminiProvider } = require('./dist/server/services/llm/providers/gemini');
