...
// Test connecting paths between 10 NCBIGene entities
const TEST_ENTITIES = [
  'NCBIGene:9244',   // CRLF1
  'NCBIGene:2022',   // ENG
  'NCBIGene:2027',   // ENO3
...
async function collect2HopNeighborhoodParallel(startEntities) {
  ...
  const level1Promises = startEntities.map(async entity => {
    const geneName = GENE_NAMES[entity] || entity;
    console.log(`  Processing ${entity} (${geneName})...`);
    try {
...
async function runConnectingPathsTest() {
  console.log('================================================================================');
  console.log('NCBIGene CONNECTING PATHS TEST');
  console.log('================================================================================');
  console.log(`Test Date: ${new Date().toISOString()}`);
  console.log(`Test Entities: ${TEST_ENTITIES.map(e => `${e} (${GENE_NAMES[e]})`).join(', ')}`);
  ...
  try {
    ...
    if (twoHopPaths.length > 0) {
      ...
      twoHopPaths.forEach((path, i) => {
        ...
        const start1Name = GENE_NAMES[path.start1] || path.start1;
        const start2Name = GENE_NAMES[path.start2] || path.start2;
        console.log(`  ${i+1}. ${start1Name} (${path.start1}) ↔ ${path.intermediates[0]} (${intermediate?.name || 'Unknown'}) ↔ ${start2Name} (${path.start2})`);
      });
    }
    ...
    if (threeHopPaths.length > 0) {
      ...
      threeHopPaths.forEach((path, i) => {
        ...
        const start1Name = GENE_NAMES[path.start1] || path.start1;
        const start2Name = GENE_NAMES[path.start2] || path.start2;
        console.log(`  ${i+1}. ${start1Name} (${path.start1}) → ${path.intermediates[0]} (${int1?.name || 'Unknown'}) → ${path.intermediates[1]} (${int2?.name || 'Unknown'}) → ${start2Name} (${path.start2})`);
      });
    }
    ...
    console.log(`\n🏁 NCBIGene connecting paths test complete!`);
    console.log('================================================================================\n');
