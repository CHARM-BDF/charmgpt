    ...
    tools: [
      {
        ...
        inputSchema: {
          ...
          required: ['entity'],
                ...
                description: 'Find network neighborhood for multiple biomedical entities. Gets bidirectional relationships for each entity and combines them.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        entities: {
                            ...
                            description: 'Array of entities to find network neighborhood for (CURIE format, e.g., ["HGNC:8651", "DRUGBANK:DB12411", "NCBIGene:4893"])',
                            minItems: 2
                        },
                    },
                    required: ['entities'],
...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
  ...
  try {
    if (name === 'get-everything') {
    ...
    } else if (name === 'network-neighborhood') {
      ...
      log(`🚨 STARTING network-neighborhood for entities: ${entities.join(', ')}`);
      ...
      await server.sendLoggingMessage({
        ...
        logger: 'medik-mcp',
        data: {
          message: `Starting network neighborhood query for ${entities.length} entities...`,
          timestamp: new Date().toISOString(),
          traceId: Math.random().toString(36).substring(2, 8)
        }
      });
      
      // Get bidirectional queries for all entities and combine them
      const allResults: MediKanrenTuple[] = [];
      
      for (const entity of entities) {
        log(`Querying entity: ${entity}`);
        const entityResults = await runBidirectionalQuery(entity);
        
        if (entityResults && entityResults.length > 0) {
          log(`Found ${entityResults.length} relationships for ${entity}`);
          allResults.push(...entityResults);
        } else {
          log(`No relationships found for ${entity}`);
        }
      ...
      if (allResults.length === 0) {
        const message = `No relationships found for entities: ${entities.join(', ')}`;
        log(`❌ ${message}`);
        ...
        return {
          content: [
            {
              type: 'text',
              text: `# No Results Found\n\nNo biomedical relationships were found for entities: **${entities.join(', ')}**\n\nThis could mean:\n- The entities don't exist in the MediKanren knowledge base\n- The entities have no known relationships\n- There was an API connectivity issue\n\nPlease verify the entity identifiers are correct (should be in CURIE format like "HGNC:8651").`
            } as TextContent
          ]
        };
      }
      ...
      await server.sendLoggingMessage({
        ...
        logger: 'medik-mcp',
        data: {
          message: `Found ${allResults.length} raw relationships, creating knowledge graph...`,
          timestamp: new Date().toISOString(),
          traceId: Math.random().toString(36).substring(2, 8)
        }
      });
      
      // Create knowledge graph combining results from all entities
      const graph = createKnowledgeGraph(allResults, entities[0]); // Use first entity as the "primary" entity
      ...
      // Create response
      const title = `Network Neighborhood: Combined relationships for ${entities.length} entities`;
      const summary = `Found ${graph.links.length} relationships connecting ${graph.nodes.length} biomedical entities.`;
      ...
      log(`✅ SUCCESS: Returning network neighborhood graph with ${graph.nodes.length} nodes and ${graph.links.length} links`);
      
      return {
        content: [
          {
            type: 'text',
            text: `# ${title}\n\n${summary}\n\n## Statistics\n- **Nodes**: ${graph.nodes.length}\n- **Relationships**: ${graph.links.length}\n- **Queried Entities**: ${entities.length}\n- **Filtered out**: ${graph.filteredCount} basic relationships, ${graph.filteredNodeCount} unreliable nodes\n\n## Entity Types Found\n${Array.from(new Set(graph.nodes.map(n => n.entityType))).map(type => `- ${type}`).join('\n')}\n\nThe knowledge graph has been generated and can be visualized using the attached artifact.`
          } as TextContent
        ],
        artifacts: [
          {
