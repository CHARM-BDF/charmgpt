 ...
 * This query excludes biolink:BiologicalEntity to see if that's causing sequence variants to appear
 */
...
// Updated test query for gene NCBIGene:283635 (FAM177A1) without BiologicalEntity
const testQuery = {
  "message": {
    "query_graph": {
      "edges": {
...
async function submitQuery() {
  console.log('🧬 Testing ARS Query v4 for NCBIGene:283635 (FAM177A1)');
  console.log('📤 Submitting query WITHOUT biolink:BiologicalEntity...');
  console.log('🎯 Target categories (excluding BiologicalEntity):');
...
async function getDetailedResults(pk) {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      const resultsFile = path.join(__dirname, `ars_results_v4_${pk}.json`);
...
async function runTest() {
  console.log('🚀 Starting ARS API Test v4');
