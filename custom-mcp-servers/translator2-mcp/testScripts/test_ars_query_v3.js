 ...
 * This query should exclude sequence variants by specifying target categories
 */
...
// Updated test query for gene NCBIGene:283635 (FAM177A1) with specific target categories
const testQuery = {
  "message": {
    "query_graph": {
      "edges": {
...
async function submitQuery() {
  console.log('🧬 Testing ARS Query v3 for NCBIGene:283635 (FAM177A1)');
  console.log('📤 Submitting query with specific target categories...');
  console.log('🎯 Target categories (excluding sequence variants):');
...
async function getDetailedResults(pk) {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      const resultsFile = path.join(__dirname, `ars_results_v3_${pk}.json`);
      ...
      if (data && data.message && data.message.knowledge_graph) {
        ...
        const edges = Object.values(kg.edges || {});
        if (edges.length > 0) {
          console.log(`\n🔗 Sample relationships:`);
          edges.slice(0, 3).forEach((edge, index) => {
            const subject = kg.nodes[edge.subject]?.name || edge.subject;
            const object = kg.nodes[edge.object]?.name || edge.object;
...
async function runTest() {
  console.log('🚀 Starting ARS API Test v3');
