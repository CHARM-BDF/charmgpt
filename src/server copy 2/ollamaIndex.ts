 ...
 */
const app = express();
const port = process.env.OLLAMA_PORT || 3001;
...
app.post('/api/chat', async (req: Request<{}, {}, { 
  ...
  try {
    ...
    while (!isSequentialThinkingComplete) {
      ...
      console.log('###DEBUG OLLAMA DATA FOR TOOL CALL - formattedTools:', formattedTools);

      // Replace the Anthropic tool call with Ollama
      const toolResponse = await ollama.chat({
        ...
        tools: formattedTools.map(tool => ({
          ...
          function: {
            ...
            parameters: {
              ...tool.input_schema,
              required: tool.input_schema.required || [] // Ensure required is always an array
            }
          }
        })) as Tool[]
      });

      console.log('###DEBUG OLLAMA DATA FOR TOOL CALL- toolResponse:', toolResponse);
      //print(response['message']['tool_calls'])


      // Transform Ollama response to match Anthropic format
      const transformedResponse = {
        content: [{
          type: 'tool_use',
          name: toolResponse.message.tool_calls?.[0]?.function?.name || '',
          input: toolResponse.message.tool_calls?.[0]?.function?.arguments || {}
        }]
      };

      // Process tool usage
      for (const content of transformedResponse.content) {
      ...
      if (!transformedResponse.content.some(c => c.type === 'tool_use')) {
        isSequentialThinkingComplete = true;
      }
    }
