...
export function DataViewer({ dataFile, height }: DataViewerProps) {
	const { activeArtifact, selectedStep, setSelectedStep, addArtifact } =
	...
	const handleSaveDataFrame = async () => {
		if (
			!selectedStep ||
			!activeArtifact?.code ||
			(activeArtifact.var2val && !activeArtifact.var2val[selectedStep])
		)
			return;
		...
		try {
			...
			await addArtifact({
				name: csvFilename,
				code:
					`# Save ${selectedStep} to ${csvFilename}\n\n` +
					activeArtifact.code,
				output: '',
				type: 'code',
				pinned: true,
		...
		>
			...
			{activeArtifact?.var2val && variables.length > 0 && (
				...
				>
					...
					{/* Add download button */}
					{selectedStep &&
						activeArtifact.var2val[selectedStep]?.type ===
							'file' && (
							<IconButton
